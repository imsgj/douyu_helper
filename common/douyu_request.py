...
class DYHTTPRequests:
    ...
    def __init__(self):
        ...
        self.session.get(url, headers=self.header)
        ck = ''
        for k, v in requests.utils.dict_from_cookiejar(self.session.cookies).items():
            ck += f'{k}={v}; '
        self.cookie = ck[:-2]
        self.header = {
            "Content-Type": "application/x-www-form-urlencoded",
...
if __name__ == '__main__':
    ...
    # DYHTTPRequests()
    login = dyreq.request("get", '/wgapi/livenc/liveweb/follow/list').json()
    print(login)
